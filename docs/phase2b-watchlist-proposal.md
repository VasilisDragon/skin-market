...
2. **Souvenir AWP Dragon Lore Battle-Scarred coverage discrepancy.** Pricempire's all-6-probe says 6/6 priced; live ingest after 13.5h still shows only 2 providers writing rows (characterized as expected for an extremely low-liquidity item per validation §2). Decide whether to keep this Tier-5 canary or replace it. Probably keep — it's only 1/8 of Tier 5, and a canary that exercises the "Pricempire claims coverage but the underlying providers serve nulls" path is arguably more valuable than one that doesn't.
