# Discord bot install and workflow

The bot runs as a docker compose service using the same image as the rest of the
stack. It makes outbound calls to Discord, the local `api` container, and
DeepSeek. It writes one `llm_usage_log` row per DeepSeek request and exposes no
inbound network port.

The bot design is in `docs/adr/016-discord-bot-runtime.md`; DeepSeek provider
selection and pricing are in `docs/adr/026-deepseek-inference-cutover.md`.

## Prerequisites

- Stack already running (`docker compose ps` should show `postgres`,
  `collector`, `analytics`, and `api` healthy or running).
- The api container's `SKIN_MARKET_API_TOKEN` set and known.
...
4. **Reset Token** -> copy the new token. Treat this as a secret on par with a password.
...
2. Right-click your username anywhere -> **Copy User ID**. This is a long decimal string.
...
PORTFOLIO_SNAPSHOT_MAX_DAILY_PER_USER=10
SIGNAL_SUBSCRIPTION_POLL_SECONDS=60
SIGNAL_SUBSCRIPTION_BATCH_LIMIT=100
SIGNAL_SUBSCRIPTION_MAX_ACTIVE_PER_USER=1
PORTFOLIO_MONITOR_POLL_SECONDS=300
PORTFOLIO_MONITOR_BATCH_LIMIT=25
PORTFOLIO_MONITOR_MAX_ACTIVE_PER_USER=1

# Already set for the API:
...
Empty `DISCORD_ALLOWED_USER_IDS` is valid config, but the bot will refuse every
message with "I'm not configured for any users yet". That is the fail-closed
default.
...
| Bot replies "I had trouble answering that. Try rephrasing?" | The LLM hit the tool-call cap without producing text | Ask a more specific question and inspect bot logs. |
...
- **Slash commands** (`/price ak-redline-ft`) are not implemented.
- **Conversation memory** is not implemented; each Discord message is independent context.
- **Per-server config** is not implemented; the bot uses one backend and allowlist.
- **Adding items via the bot** is not implemented; use `scripts/watchlist_edit.py`.
- **LLM fallback** is not implemented; if DeepSeek is unavailable, the bot says so.
...
- `docs/adr/015-bot-skill-design.md` - bot rendering rules.
- `docs/archive/bot_skill_hermes_attempt/` - archived experimental runtime.
